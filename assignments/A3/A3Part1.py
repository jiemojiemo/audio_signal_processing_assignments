...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    ## Your code here
    # calculate M
    num_samples_each_period_1 = fs/f1
    num_samples_each_period_2 = fs/f2
    GCD = gcd(num_samples_each_period_1, num_samples_each_period_2)
    
    M = int(num_samples_each_period_1*num_samples_each_period_2/GCD)
    ...
    # calculating the magnitude of the positive side
    mX = 20 * np.log10(np.abs(X[:(M//2)+1]))
    
    # set 0 if mX under the threshold
    #tol_db = -120
    #mX[ mX < tol_db ] = 0
