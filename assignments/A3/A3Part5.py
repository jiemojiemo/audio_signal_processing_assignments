...
def zpFFTsizeExpt(x, fs):
    ...
    ## Your code here 
    # Case-1: Input signal xseg (256 samples), window w1 (256 samples), and FFT size of 256
    mX, pX = dftAnal(xseg, w1, 256)
    mX1_80 = mX[:80]
    
    # Case-2: Input signal x (512 samples), window w2 (512 samples), and FFT size of 512
    mX, pX = dftAnal(x, w2, 512)
    mX2_80 = mX[:80]
    
    # Case-3: Input signal xseg (256 samples), window w1 (256 samples), and FFT size of 512
    mX, pX = dftAnal(xseg, w1, 512)
    mX3_80 = mX[:80]
