...
def optimalZeropad(x, fs, f):
    ...
    """
    ## Your code here
    # get the lenght of input signal
    M = x.size
    
    num_samples_each_period = int(fs/f)
    
    # calculate the N
    N = np.ceil(M/num_samples_each_period) * num_samples_each_period
    N = int(N)
    ...
    # calculate the magnitude of the positive side
    mX = 20 * np.log10(np.abs(X[:(N//2) + 1]))
    
    # set 0 if the mX lower than the threshold
    #tol_db = -120
    #mX[ mX < tol_db ] = .0
